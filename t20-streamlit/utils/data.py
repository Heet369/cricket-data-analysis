...
def get_batting_agg(batting_df, group_col='batsmanName'):
    agg = batting_df.groupby(group_col).agg({
        'runs': 'sum',
        'balls': 'sum',
        'innings': lambda x: x.nunique(),
        '4s': 'sum',
        '6s': 'sum',
        'out/not_out': lambda x: (x == 'out').sum(),
        'match_id': 'count'
    }).reset_index()
    agg.columns = ['batsmanName', 'total_runs', 'total_balls', 'innings', 'fours', 'sixes', 'dismissals', 'matches']
    agg['not_out'] = agg['matches'] - agg['dismissals']
...
def get_bowling_agg(bowling_df, group_col='bowlerName'):
    ...
    agg.columns = ['bowlerName', 'overs', 'runs_conceded', 'wickets', 'maidens', 'avg_economy', 'matches']
